...
- **Account Settings & Security Flow:** Built a dedicated multi-page security architecture (`/settings/security`) with chevron list rows and dedicated action sub-pages (`change-password`, `email`, `two-factor`, `sessions`, `delete-account`), featuring dedicated validation and full-width action buttons.
