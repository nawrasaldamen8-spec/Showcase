...
export const PostCard: React.FC<PostCardProps> = ({ post, spanType = "square", className = "", aspectRatio }) => {
  ...
  const handleCardClick = () => {
    const isSearch = location.pathname.startsWith("/feed/search") || location.pathname.startsWith("/search");
    const isProfile = location.pathname.startsWith("/u/");
    const fromLabel = isSearch
      ? "Search Results"
      : isProfile
        ? "Profile"
